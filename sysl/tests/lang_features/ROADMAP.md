...
| `generic_alias_basic.lsysl` 🟢 | `type GabUnary[T] = (T) -> T` as parameter type; two-param alias `(A,A)->B`; alias as struct field type (6 tests). Field-type gap fixed at sysl@1194e72da. |
...
| `defer_no_return.lsysl` 🟢 | single defer + implicit exit; defer-then-body; defer in always-taken if-branch fires at fn exit; **defer in skipped if-branch does NOT fire**; no-defer baseline (6 tests). Skipped-branch defer fixed at sysl@e95bfe65a. |
...
| `closure_recursive_inner_def.lsysl` 🟢 | single-fn self-recursive inner `def`: factorial (3), fib (2), capture-from-outer-scope (1), two unrelated self-recursive inner defs (1), three unrelated self-recursive inner defs (1) — 8 tests. SVM two-defs panic eliminated by the chunk-3 closure-env fixes (sysl@8551e6777 / be0ec3246 / ed7eb3d89). |
...
| `iface_in_iface.lsysl` 🟢 | Iface composition — methods whose signature mentions another iface. `Producer.make() -> Source` (return-position iface from iface method, single + two-producer-independence + p.make().next() chain); `Sink.accept(src: Source) -> int` (iface param to iface method, single + three-times + two-source); end-to-end producer.make → sink.accept. 7 tests, no bugs surfaced — coverage gap fill. |
| `iface_closure_capture.lsysl` 🟢 | Closure captures an iface-typed local that outlives the constructing frame. Without the fix the captured iface descriptor's `data_ptr` pointed at the dead source struct on the constructing frame's stack → UAF on llvm-host (libmalloc) + trisc (bump-pool). Fix at sysl@c353e7b86: post-pass `rewriteEscapingClosureCaptureOwns` in analyzer flips `TInterfaceBox(_,_,owns=false)` → `owns=true` for any iface var captured by any closure, so codegen heap-copies the source at the box site. Mutating-self via iface method receivers unaffected (`iface_box_lifetime` 19/19 green ×3 backends after fix). 5 tests: captured-once, captured-many-calls, two-independent-factories, inline-iface, pair-of-iface-closures. |
...
| File | Tests pinned |
|---|---|
| `type_alias_basic.lsysl` 🟢 | plain + generic + chained alias; fn-type alias; pointer alias; re-param of generic enum; **two-step composition** via aliased function types (10 tests). TRISC fnptr-fnptr-scalar bug fixed at sysl@392cca670. |
...
| File | Tests pinned |
|---|---|
| `enum_simple_int.lsysl` 🟢 | ::First / ::Last / ::Pos / ::Val / ::Image / ::Succ / ::Pred; pattern match (via local-bound scrutinee + via fn param after match-on-param fix at sysl@a013c330a); two distinct enums coexist; ::Value(string) round-trip via SVM string-eq fix at sysl@72f5555fd (37 tests). |
...
| `enum_data_recursive.lsysl` 🟢 | direct recursion shape — analyzer auto-wraps `field: Self` as `&Self`, bare-variant constructor args (`Node(1, Leaf, Leaf)`) are auto-promoted to `new`. Covers construction at depth 2 (bare + explicit-`new` inner), match distinguishes Leaf vs Node, match-binds payload, value-param top-level reads, `&Self`-param recursive sum / depth / cons-list length & sum at deep nesting (20 tests). Wide multi-backend recursive-walk bug fixed at this revision. |
...
| `enum_data_iface_field.lsysl` 🟢 | Data-enum variant whose payload is an `interface` type. Single-payload `SedifMaybe { SedifJust(payload: I), SedifNone }`; two-field variant `(payload: I, tag: int)` + positional sanity with iface in second slot; recursive enum `SedifTree { SedifLeaf, SedifNode(payload: I, children: []SedifTree) }`. Two analyzer fixes: `latestEnum`/`latestInterface` refreshers + iface auto-box at all three variant-construction sites (CallAST/`new`/generic) with `owns=true`. (8 tests) |
