/* Minimal freestanding libc for sysl bare-metal RV.
 *
 ...
 *   abort, exit.
 *
 * Plus the LLVM byte-memset intrinsic (`llvm.memset.p0i8.i64`), which the
 * codegen emits as a direct intrinsic call.
 *
 * The printf engine is small but handles every format string sysl produces:
 ...
 *
 * The bump allocator is a single-arena LIFO: malloc bumps a pointer; free is
 * a no-op. Adequate for std/ tests; chunk 4 may swap in something smarter if
 * a test really needs reclamation. The arena lives between __heap_start and
 * __heap_end (defined in the linker script).
 */
