...
object MakeX86RamdiskMain:
  ...
  def main(args: Array[String]): Unit =
    ...
    // Phase 1 chunk 9 fixture: ship the dynamic linker (ld-musl-x86_64.so.1)
    ...
    // same here at ramdisk-pack time, sourcing from slix/build-musl-x86/lib/libc.so
    // (produced by slix/build-musl-x86.sh with --enable-shared). If the file
    // isn't present, the ramdisk just won't include it; dynamically-linked
    // tests fail but static binaries are unaffected.
    val ldMuslPath = Paths.get("slix/build-musl-x86/lib/libc.so")
    val ldMuslBytes: Option[Array[Byte]] =
      if Files.exists(ldMuslPath) then
        val bs = Files.readAllBytes(ldMuslPath)
        System.err.println(s"  including /lib/ld-musl-x86_64.so.1 (${bs.length} bytes, from $ldMuslPath)")
        Some(bs)
      else
        System.err.println(s"  WARN: $ldMuslPath not found — /lib/ld-musl-x86_64.so.1 omitted")
        None
    ...
    // link time (-dynamic-linker /lib/ld-musl-x86_64.so.1). Shared
    ...
    // phello-style link against -lm doesn't double-load libc.
    val files: Map[String, Array[Byte]] =
      binFiles
        + ("/test.tar" -> tarBytes)
        ++ ldMuslBytes.map("/lib/ld-musl-x86_64.so.1" -> _).toMap
    ...
    val disk = TFS.format(
      blockSize = 4096,
      totalBlocks = 32768, // 128 MB — well under TFS' ~256 MB cap (65535 × 4096); kept in lockstep with aarch64
      maxInodes = 1024,
      prefill = prefill,
      files = files,
    )
