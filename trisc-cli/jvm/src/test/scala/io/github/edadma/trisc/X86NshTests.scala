...
class X86NshTests extends AnyFreeSpec with Matchers with BeforeAndAfterEach {
  ...
  "musl: ppoll/pselect sigmask atomicity + EINTR" in {
    // Mirror of the aarch64 mppm test — see slix/test/ppoll_mask.c.
    // Blocks SIGUSR1, raises it under the blocked mask, then calls
    // ppoll with an empty sigmask: the kernel must unblock SIGUSR1
    // atomically for the wait, deliver the handler frame, return
    // -1/EINTR, and restore the original blocked mask. The second
    // pass repeats the sequence through pselect6. Handler counter
    // is cumulative across the two waits.
    qemu.send("mppm\n")
    val output = qemu.waitFor("mppm: done")
  ...
  "musl: SYS_futex WAIT/WAKE + EAGAIN/ETIMEDOUT/WAIT_BITSET" in {
    // Mirror of the aarch64 mfx test — see slix/test/futex_basic.c.
    // Five SYS_futex shapes in one dyn-linked process: empty WAKE,
    // EAGAIN on mismatched WAIT precondition, ETIMEDOUT on
    // short-relative-timeout WAIT, fork+WAKE cross-page-table
    // round-trip (PA hash table finds the parent waiter through the
    // child's PTBR), and ETIMEDOUT on short-absolute WAIT_BITSET.
    qemu.send("mfx\n")
    val output = qemu.waitFor("mfx: done")
