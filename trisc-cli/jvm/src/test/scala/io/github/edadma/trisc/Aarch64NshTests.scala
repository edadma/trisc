...
class Aarch64NshTests extends AnyFreeSpec with Matchers with BeforeAndAfterEach {
  ...
  "musl: signalfd4 + read + epoll integration" in {
    // msignalfd (slix/test/signalfd.c) blocks SIGUSR1/SIGUSR2 via
    // sigprocmask, creates a signalfd watching both, then exercises:
    // empty-read EAGAIN, raise+read drains one signalfd_siginfo with
    // ssi_signo==SIGUSR1, epoll_wait reports EPOLLIN after a second
    // raise, draining then re-polling returns 0, and a final
    // two-at-once raise() pair fills a 256-byte buffer with both
    // signos in ascending order.
    val output = qemu.command("/bin/msignalfd")
  ...
  "musl: SYS_clone spawns a new thread sharing the parent's VM" in {
    // mclone (slix/test/clone_basic.c) calls __clone() directly with
    // CLONE_VM | CLONE_THREAD | CLONE_PARENT_SETTID. The child writes
    // a shared volatile global (1 | 0xCA11ED), then SYS_exits.
    // The parent's spin loop observes the write (proving CLONE_VM is
    // honoured) and reads back *ptid (proving CLONE_PARENT_SETTID
    // published the new tid). Sub-thread SYS_exit must not kill the
    // process — that's what terminate_current's sub-thread vs
    // last-thread distinction guards against.
    val output = qemu.command("/bin/mclone")
  ...
  "musl: pthread_create + pthread_join end-to-end" in {
    // mpb (slix/test/pthread_basic.c) spawns 4 threads through real
    // musl pthread_create — each worker writes 0x1000+idx into both
    // a shared array slot and its own __thread mirror, then returns.
    // Main pthread_joins all four. The join only returns when the
    // CLONE_CHILD_CLEARTID futex-wake in terminate_current fires;
    // mpb hangs forever otherwise. Sums:
    //   slots = 4*0x1000 + (0+1+2+3) = 16390
    //   rets  = sum of return values (cast through uintptr_t) = 16390
    //   main_local: untouched by workers (each had its own TLS) = 0
    val output = qemu.command("/bin/mpb")
  ...
  "musl: open(O_CREAT) + write + readback (Phase 4 chunk 4)" in {
    ...
    // routes to VFS_CMD_CREATE_C with TFS ftype=REG and the
    // mode argument's perm bits, then re-opens to obtain the
    // handle. Verifies write+close+stat+read round-trip and
    // that O_CREAT|O_EXCL on an existing file returns -EEXIST
    // (errno=17) without leaking a VFS handle.
    qemu.send("mfcreat\n")
  ...
  "execve: replace process image" in {
    ...
    // the saved ELR/SP_EL0 via svc_execve_finalize so the calling thread
  ...
  "execve: dynamically-linked hello (PT_INTERP + ld-musl)" in {
    ...
    // /lib/ld-musl-aarch64.so.1, which PM detects in pm_handle_execve
  ...
  "execve: printf via libc.so under /usr/lib" in {
    ...
    // which our compiler_rt stubs (chunk 9) trap on. Real builtins
    // are a known follow-up; %d sidesteps fmt_fp.
  ...
  "nsh: spawn /bin/dhello (PIE through pm_handle_spawn_inner)" in {
    ...
    // /bin/dhello is the same PIE binary test_dhello execve's into;
    // running it from nsh exercises pm_handle_spawn_inner → the
    // shared pm_load_image_pie helper. If this faults `!K:` at an
    // entry-offset address, the spawn path lost the PIE rebase /
    // PT_INTERP / rich-auxv shape that execve uses.
