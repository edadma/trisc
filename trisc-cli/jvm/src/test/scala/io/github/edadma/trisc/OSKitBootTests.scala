...
class OSKitBootTests extends OSKitTestHelpers {
  ...
  "TOS compiles and links" in {
    ...
    val result = driver.compile(Map(
      ...
      "oskit/arch/trisc/vm" -> LiterateRenderer.tangle(new LiterateParser().parse(scala.io.Source.fromFile("oskit/arch/trisc/vm.lsysl").mkString)),
      "oskit/arch/trisc/cpu" -> LiterateRenderer.tangle(new LiterateParser().parse(scala.io.Source.fromFile("oskit/arch/trisc/cpu.lsysl").mkString)),
